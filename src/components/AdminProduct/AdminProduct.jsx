...
cloudinary.config({
    ...
    api_key: '889792192213277', 
    api_secret: 'UTYXeHioJZHG75DtzGqzqwbvDdc',
});
...
const AdminProduct = () => {
    ...
    const [stateProductDetails, setStateProductDetails] = useState({
        ...
        // gửi data đến server, nhận lại res
        const mutation = useMutationHooks(
            (data) => {
                const {name,price,description,rating,image,countInStock,type} = data
                const res = ProductService.createProduct({name,price,description,rating,image,countInStock,type});
                return res;
            }
        );
    ...
    //trạng thái của mutation trong create product
    ...
    useEffect(() => {
        console.log("Data deleted response:", dataDeleted);
    ...
    // e: tên của input field đang thay đổi phải trùng với một key trong stateProduct
    ...
    const handleOnChangeAvatar = async ({ fileList }) => {
        ...
        formData.append('upload_preset', 'uploadNhom3'); // Thay bằng preset của bạn

        try {
            const res = await fetch('https://api.cloudinary.com/v1_1/dc8sfvgov/image/upload', {
                method: 'POST',
                body: formData,
            });
            const data = await res.json();

            // Lưu URL ảnh tải lên thành công vào state
            setStateProduct({
                ...stateProduct,
    ...
    //Khung ngang thông tin sản phẩm
    const columns = [
        {
          title: 'Name',
          dataIndex: 'name',
